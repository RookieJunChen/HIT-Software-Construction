...
public class Difference<E> {
   ...
   * constructor.
   * 
   * @param c1 ThinginTrack
   * @param c2 ThinginTrack
   */
  public Difference(List<ThinginTrack<E>> c1, List<ThinginTrack<E>> c2) {
    ...
    for (i = 0; i < min; i++) {
      orbitdifferences.add(new orbitdifference<E>(i + 1, c1.get(i).getthingsintrack(),
          c2.get(i).getthingsintrack()));
    }
    checkRep();
  }
...
class orbitdifference<E> {
  ...
  // Abstraction function:
  // orbitnum对应轨道层数，thingnumdif对应轨道上物体数量差;
  // orbitunique1对应独属于轨道一的物体，orbiunique2对应独属于轨道二的物体
  ...
  public String toString() {
    ...
    if ((orbitunique1.size() != 0 || orbitunique2.size() != 0)
        && !p.getname().equals("Electronics")) {
      str.append("{");
